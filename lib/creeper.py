...
class Creeper(object):
    ...
    def get(self,url='',headers='',params='',proxies='',stream=False):
        ...
        response = self.__response(work='get',url=self.url,headers=self.headers,params=self.params,proxies=self.proxies,stream=stream)
        if response != False:
    ...
    def post(self,url='',headers='',params='',data='',proxies=''):
        ...
        if proxies != '':
            self.set_proxies(proxies)
        response = self.__response(work='post',url=self.url,headers=self.headers,params=self.params,proxies=self.proxies)
        if response != False:
            self.response = response
            return response
        else:
    ...
    def __response(self,work,url,headers='',params='',data='',proxies='',stream=False):
        ...
        for times in range(1,max_try_times+1):
            try:
                if work == 'get':
                    print(proxies)
                    response = requests.get(url, timeout = wait_time, headers=headers, params=params, stream=stream, proxies=proxies)
                elif work == 'post':
